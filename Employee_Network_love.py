...
exec_list = ['CEO','President','Vice President','Managing Director','Manager']
...
email_df['cleaned_body'] = [str(x) for x in email_df['cleaned_body']]
word_mask = ['private' in email for email in email_df['cleaned_body']]

email_df = email_df[word_mask]

print(len(email_df))
new_mids = list(email_df['mid'])
...
edge_thresh = np.percentile(edge_values,12)
...
ranks = ['CEO','President','Vice President','Director','Managing Director','Manager']
...
plotly_color = ['salmon','burlywood','cornsilk','goldenrod','honeydew','linen','honeydew','black','black']
...
fig = go.Figure(data=[edge_trace, node_trace],
             layout=go.Layout(
                title='<br>Enron Graph "private" Emails ',
                titlefont_size=16,
                showlegend=False,
                hovermode='closest',
